...
FROM
	vd_pedidos PED
LEFT JOIN pv_financeiro pf ON
	PED.pk_id = PF.FK_ORIGEM
LEFT JOIN es_movimentos MOV ON
	PED.pk_id = MOV.FK_ORIGEM
LEFT JOIN tb_produtos AS TP ON
	TP.pk_id = MOV.FK_PRODUTO
LEFT JOIN tb_familias AS TF ON
	TF.pk_id = TP.FK_FAMILIA
LEFT JOIN pv_tefbandeira AS BAN ON
	BAN.cd_bandeira = PF.cd_tefbandeira
LEFT JOIN pq_ingresso AS PC ON
	PC.FK_ORIGEM = mov.FK_ORIGEM
	AND PC.FK_ORIGEMITEM = MOV.PK_ID
LEFT JOIN pq_catraca AS PC2 ON
	PC.NR_PASSAPORTE = PC2.nr_passaporte
	AND PC.NR_PASSAPORTEDIG = PC2.NR_PASSAPORTEDIG
LEFT JOIN fi_movfin AS FM ON
	FM.PK_ID = PF.FK_MOVFIN
WHERE
		mov.TG_ORIGEM = 'V'
